...
- 门禁沿 0.13.0 基线：`cargo test --workspace --locked` 1,635 passed / 0 failed（本仓实测 2026-08-12；规范仓 2026-08-10 基线记录同命令实测 1,629，见规范仓 CHANGELOG "Verified（2026-08-10 实测）" 节，数字随基线后测试演进）、18 套语言无关 suite 519/519（聚合 digest cfd6e296…）、semver 基线为拆分前 v0.8.0 crates/ 树；
- CI（ci.yml）11 job + check 聚合：lint / test / coverage（硬下限 + 趋势门禁）/ msrv / conformance / deny / audit / semver / package / check-version-consistency / examples；
