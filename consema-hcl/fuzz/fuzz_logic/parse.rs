// Fuzz-target logic: consema-hcl parser entry point (0.13.0 gate plan M2).
//
...
// wrapper (`crates/consema-hcl/fuzz/fuzz_targets/parse.rs`). Resource
...
/// Drives the production HCL profiles over one input.
pub fn fuzz_parse(data: &[u8]) {
    for profile in [HclProfile::NativeV1, HclProfile::TfvarsV1] {
        assert_parse_closure(data, profile, HclParseLimits::default());
    }
}
